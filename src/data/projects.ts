...
export const projects: Project[] = [
  {
    ...
    fullContent: {
      ...
      solution: "Foi desenvolvido com python, um executável que realiza automaticamente o  processo de ingestão e tratamento de dados a partir de múltiplas bases em Excel.\n\nA solução realiza a extração de CNPJs elegíveis com base em regras de negócio e faz o enriquecimento dos dados por meio de merge com uma base de referência contendo atributos cadastrais como CPF, grupo e segmento.\n\nTambém foram implementadas regras de qualidade de dados, identificando CNPJs não encontrados na base de referência e CPFs com renda inconsistente. A partir disso, é gerados dois arquivos: um arquivo de input para processamento no Databricks e um arquivo de rejeitos para controle de inconsistências e governança dos dados.\n\nNa camada de transformação, foi construída uma pipeline no Databricks com integração entre múltiplas tabelas, aplicação de regras de negócio, ETL, padronização de schema, deduplicação e atualização incremental em tabela analítica.\n\nNa camada analítica, foram desenvolvidos dashboards em Power BI com foco em monitoramento e tomada de decisão, incluindo indicadores como volume de CPFs analisados, taxa de aceitação de cartão, média de limite concedido, alavancagem, segmentações, análise por canal e analise de comportamento de aceitação.",
  ...
  },
  {
    ...
    introPhrase: "Neste projeto, desenvolvi uma solução para reestruturar o processamento de uma base crítica  substituindo um fluxo legado limitado por uma arquitetura mais robusta e escalável.",
    fullContent: {
  ...
  },
  {
    ...
    fullContent: {
      ...
      solution: "Foi desenvolvida uma solução de data reconciliation de dados entre duas fontes: uma base em Excel contendo tudo o que havia sido solicitado para ser cadastrado no sistema e a base de campanhas, que representa o que está efetivamente cadastrado no sistema.\n\nA solução realizava ingestão, padronização e comparação entre os dados, verificando se cada solicitação havia sido cadastrada, se ainda não havia sido cadastrada ou, caso cadastrada, se os parâmetros estavam corretos.\n\nNos casos de inconsistência, o relatório final aponta exatamente onde estava o erro, facilitando a correção e aumentando a rastreabilidade operacional.\n\nO output consiste em um relatório analítico de riscos operacionais relacionado a campanhas e convênios, com detalhamento completo do status e das divergências encontradas.",
      ...
      technologies: "Python, Excel, técnicas de validação e data reconciliation",
    },
  },
  {
    ...
    fullContent: {
      ...
      solution: "Foi desenvolvido em Python e empacotado em formato de aplicativo, com interface intuitiva, logs de erro e barra de progresso.\n\nA automação passou a manipular automaticamente as bases necessárias e gerar todos os arquivos prontos para abertura, exigindo apenas que o colaborador coloque as bases na mesma pasta do aplicativo e execute o processo com poucos cliques.\n\nO robô realiza a  leitura das bases, o tratamento dos dados, a montagem dos arquivos finais e a organização dos outputs.",
